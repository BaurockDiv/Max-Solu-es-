...
const getInitialFollows = () => {
  try {
    const saved = localStorage.getItem('bizstream_follows');
...
const supabaseHelpers = {
  ...
  toggleFollow: async (businessId: string, userId?: string) => {
    ...
    try {
      ...
      localStorage.setItem('bizstream_follows', JSON.stringify(Array.from(followedBusinessIds)));
    } catch (err) {
  ...
  syncFollows: async (userId: string) => {
    try {
      ...
      if (data) {
        ...
        localStorage.setItem('bizstream_follows', JSON.stringify(dbIds));
      }
    } catch (err) {
