      ...
      <div className="p-6 pb-4 space-y-4 shrink-0 bg-white dark:bg-black z-10">
        ...
        <div className="relative group">
          ...
          <input 
            ...
            className="w-full bg-zinc-100 dark:bg-zinc-900 rounded-[1.5rem] py-5 pl-12 pr-12 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all border-none placeholder:text-zinc-400 text-zinc-900 dark:text-white"
            value={search}
        ...
        {showAdvancedFilters && (
          <div className="p-5 bg-zinc-50 dark:bg-zinc-950 rounded-[2rem] border border-zinc-100 dark:border-zinc-900 animate-in slide-in-from-top-2 duration-300 space-y-5 shadow-sm">
            <div className="grid grid-cols-2 gap-3">
                <button 
                    onClick={() => setOnlyVerified(!onlyVerified)}
                    className={`py-3 px-4 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-2 border transition-all ${onlyVerified ? 'bg-blue-600 border-blue-600 text-white shadow-lg' : 'bg-white dark:bg-black border-zinc-200 dark:border-zinc-800 text-zinc-400'}`}
                >
                    <CheckCircle2 size={14}/> Verificados
                </button>
                <button 
                    onClick={() => setSortBy(sortBy === 'name' ? 'verified' : 'name')}
                    className={`py-3 px-4 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-2 border transition-all bg-white dark:bg-black border-zinc-200 dark:border-zinc-800 text-zinc-400`}
                >
            ...
            <div className="relative">
                ...
                <input 
                    ...
                    className="w-full bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-xl py-3 pl-9 pr-4 text-[11px] font-bold text-black dark:text-white outline-none focus:ring-1 focus:ring-blue-500"
                    value={locationFilter}
                    onChange={(e) => setLocationFilter(e.target.value)}
                />
            </div>
            
            <button onClick={clearFilters} className="w-full text-[9px] font-black text-blue-600 uppercase tracking-widest pt-1">Redefinir Filtros</button>
          </div>
        )}
      </div>
        ...
        <div className="p-6 pt-0 space-y-8 pb-32">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-1">
              <h2 className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.3em]">Explorar Segmentos</h2>
            </div>
            <div className="flex overflow-x-auto gap-3 hide-scrollbar pb-2 -mx-1 px-1">
              {categories.map((cat) => (
                <button 
                  ...
                  className={`h-24 min-w-[120px] rounded-[1.8rem] p-4 flex flex-col justify-end relative overflow-hidden active:scale-95 transition-all cursor-pointer border-2 ${selectedCategory === cat ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-zinc-100 dark:border-transparent bg-zinc-50 dark:bg-zinc-900 shadow-sm'}`}
                >
                  <Grid size={18} className={selectedCategory === cat ? 'text-blue-500' : 'text-zinc-300 dark:text-zinc-600'} />
                  <span className={`text-[9px] font-black leading-tight mt-2 uppercase tracking-tighter text-left ${selectedCategory === cat ? 'text-blue-700 dark:text-blue-300' : 'text-zinc-500 dark:text-zinc-400'}`}>
                    {cat}
                  </span>
                </button>
              ))}
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                <div 
                  ...
                  className="flex items-center p-4 rounded-[2.2rem] border border-zinc-100 dark:border-zinc-800 hover:border-blue-500/20 active:scale-[0.98] transition-all bg-white dark:bg-zinc-900 shadow-sm group"
                >
                  <div className="relative">
                    <img src={biz.logo || 'https://picsum.photos/200/200'} className="w-16 h-16 rounded-[1.6rem] object-cover shadow-lg border border-zinc-100 dark:border-zinc-800" alt={biz.name} />
                    {biz.verified && (
                  ...
                  <div className="ml-5 flex-1 space-y-1">
                    <div className="flex items-center gap-2">
                      <h3 className="text-[13px] font-black text-zinc-900 dark:text-white uppercase tracking-tight">{biz.name}</h3>
                    </div>
                    <p className="text-[10px] text-zinc-500 font-medium line-clamp-1 opacity-80">{biz.bio || 'Profissional Certificado Max Company'}</p>
                    <div className="flex items-center gap-3">
                      <span className="text-[8px] font-black text-blue-500 uppercase tracking-widest">{biz.category.split('&')[0]}</span>
                      <div className="w-1 h-1 bg-zinc-200 dark:bg-zinc-700 rounded-full" />
                      <div className="flex items-center gap-1 text-[8px] text-zinc-400 font-black uppercase tracking-widest">
                        <MapPin size={10} className="text-zinc-300" />
                        <span>{biz.location?.split(',')[0] || 'Remoto'}</span>
                      </div>
                    </div>
                  </div>
                  <div className="w-10 h-10 rounded-2xl bg-zinc-50 dark:bg-zinc-800 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors">
                    <ChevronRight size={18} className="text-zinc-300 group-hover:text-white" />
                  </div>
                </div>
              )) : (
