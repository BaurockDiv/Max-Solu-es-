...
const DashboardView: React.FC<DashboardViewProps> = ({ business, userPosts: initialPosts }) => {
  ...
  return (
    <div className="p-6 space-y-10 bg-white dark:bg-black transition-colors duration-500 pb-32">
      ...
      <div className="p-8 bg-zinc-50 dark:bg-zinc-900 rounded-[3rem] space-y-8 border border-zinc-100 dark:border-zinc-800 shadow-sm">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-5">
          {posts.length > 0 ? posts.map(post => (
            <div key={post.id} className="flex items-center gap-6 p-6 bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[2.5rem] shadow-sm group">
            ...
            <div className="p-12 text-center border-2 border-dashed border-zinc-100 dark:border-zinc-800 rounded-[3rem]">
              <AlertCircle size={40} className="mx-auto text-zinc-200 dark:text-zinc-800 mb-4" />
...
const StatCard: React.FC<{ icon: React.ReactNode; label: string; value: string; change: string; active?: boolean; onClick?: () => void }> = ({ icon, label, value, change, active, onClick }) => (
  <div onClick={onClick} className={`p-8 rounded-[3rem] space-y-3 transition-all cursor-pointer border-2 ${active ? 'border-blue-600 bg-white dark:bg-zinc-900 shadow-2xl' : 'bg-zinc-50 dark:bg-zinc-900 border-zinc-100 dark:border-zinc-800 shadow-sm'}`}>
    <div className="flex items-center justify-between">
      <div className={`p-3 rounded-2xl shadow-sm bg-white dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700`}>{icon}</div>
